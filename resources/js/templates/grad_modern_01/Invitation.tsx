...
export default function GradStory01({ invitation, rsvpUrl }: Props) {
    ...
    const mapsQuery = useMemo(() => {
        const q = [invitation.venue_name, invitation.venue_address]
            .filter(Boolean)
            .join(', ');
        return q || '';
    ...
    const dressTags = Array.isArray(settings.dress_code_tags)
        ? settings.dress_code_tags
        : [];
                        ...
                        <div className="rounded-3xl border bg-card/60 p-6 backdrop-blur sm:p-8">
                            {primaryMessage.length ? (
                                ...
                                <p
                                    ...
                                    className="text-muted-foreground"
                                >
                                    Agrega textos complementarios para mostrar
                                    un mensaje largo aquí.
                                </p>
                            )}
                            ...
                            {dressTags.length > 0 ? (
                                <div className="flex flex-wrap gap-2">
                                    {dressTags.map((tag, idx) => (
                                        <span
                                            key={idx}
                                            className="text rounded-full border bg-background/70 px-3 py-1 font-medium dark:bg-background/40"
                                        >
                                            {tag}
                                        </span>
                                    ))}
                                ...
                                <div className="text-base">{dressFallback}</div>
                            ) : (
                ...
                {safe.length > 1 ? (
                    <>
                        <button
                            type="button"
                            onClick={goPrev}
                            className="absolute top-1/2 left-3 -translate-y-1/2 rounded-full border bg-background/70 px-3 py-2 text-sm font-medium backdrop-blur hover:bg-background"
                            aria-label="Anterior"
                        >
                            ‹
                        </button>
                        <button
                            type="button"
                            onClick={goNext}
                            className="absolute top-1/2 right-3 -translate-y-1/2 rounded-full border bg-background/70 px-3 py-2 text-sm font-medium backdrop-blur hover:bg-background"
